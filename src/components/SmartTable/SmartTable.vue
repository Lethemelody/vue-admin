	...
	<div class="content">
		<el-card class="search-box">
					...
					<el-form :inline="true" :model="searchForm" class="search-form">
						<el-form-item class="type-select">
							<el-select v-model="searchForm.type" placeholder="快捷搜索" @change="searchTypeChange">
								<template v-for="(item,index) in entityFileds" :key="item.prop">
									<el-option v-if="item.canDefaultSearch" :label="item.label" :value="item.prop" />
								</template>
							</el-select>
						</el-form-item>
						<el-form-item>
							...
							<template v-if="curSearchFiled.type == '1' || curSearchFiled.type == '2'">
								<el-input v-model="searchForm.value" type="number"
									:placeholder="'请输入'+curSearchFiled.label" class="value-filed" />
							</template>
							<template v-else-if="curSearchFiled.type == '3' || curSearchFiled.type == 6">
								<el-input v-model="searchForm.value" :placeholder="'请输入'+curSearchFiled.label"
									class="value-filed" />
							</template>
							<template v-else-if="curSearchFiled.type == '4'">
								<el-date-picker v-model="searchForm.value" :type="curSearchFiled.dateType"
									:placeholder="'请选择'+curSearchFiled.label" :format="curSearchFiled.format"
							...
							<template v-else-if="curSearchFiled.type == '5'">
								...
								<el-tree-select v-model="searchForm.value" :loading="curSearchFiled.loading" filterable
									:placeholder="'请选择'+curSearchFiled.label" lazy  node-key="id" :load="loadDataByFiled" show-checkbox
							...
							<template v-else-if="curSearchFiled.type == '8'">
								<el-select v-model="searchForm.value" :placeholder="'请选择'+curSearchFiled.label"
									class="value-filed">
									<template v-for="(item,index) in curSearchFiled.dicts" :key="item.value">
										<el-option :label="item.label" :value="item.value" />
									</template>
								</el-select>
							</template>
							<template v-else-if="curSearchFiled.type == '9'">
								<el-time-picker v-model="searchForm.value" :type="curSearchFiled.dateType"
									:placeholder="'请选择'+curSearchFiled.label" :format="curSearchFiled.format"
									class="value-date-filed" />
							</template>
						</el-form-item>
						<el-form-item>
							<el-button-group>
								...
								<el-button type="danger" @click="refresh" icon="Refresh">刷新</el-button>
								<el-button type="primary" @click="showSearchFilterBox" icon="Filter">筛选
								</el-button>
							</el-button-group>
						</el-form-item>
					</el-form>
			...
			<div class="search-filter-box" v-if="searchFilterBox">
				...
				<el-form :model="searchFilterForm" label-width="100px" class="search-filter-form">
					<div class="search-filter-form-flex">
						<template v-if="searchFilterForm" v-for="(item,index) in entityFileds" :key="item.prop">
							<template v-if="item.canSearch">{{searchFilterForm[item.prop].value}}
								<!-- 类型:1整数，2小数，3字符串，4日期，5关联对象，6超链接，7图片，8数据字典，9时间 -->
								<div class="search-filter-form-item" v-if="item.type == '1' || item.type == '2'">
									<el-form-item :label="item.label+'：'" :prop="item.prop">
										<el-input v-model="searchFilterForm[item.prop]" type="number"
											:placeholder="'请输入'+item.label" />
									</el-form-item>
								</div>
								<div class="search-filter-form-item" v-else-if="item.type == '3' || item.type == '6'">
									<el-form-item :label="item.label+'：'" :prop="item.prop">
								...
								<div class="search-filter-form-item" v-else-if="item.type == '4'">
									<el-form-item :label="item.label+'：'" :prop="item.prop">
										<el-date-picker v-model="searchFilterForm[item.prop]" :type="item.dateType"
								...
								<div class="search-filter-form-item" v-else-if="item.type == '5'">
									<el-form-item :label="item.label+'：'" :prop="item.prop">
										...
										<el-tree-select v-model="searchFilterForm[item.prop]" :loading="item.loading" filterable
											:placeholder="'请选择'+item.label" node-key="id" lazy :load="loadDataByFiled" show-checkbox
								...
								<div class="search-filter-form-item" v-else-if="item.type == '9'">
									<el-form-item :label="item.label+'：'" :prop="item.prop" s>
										<el-time-picker v-model="searchFilterForm[item.prop]" :type="item.dateType"
											:placeholder="'请选择'+item.label" class="value-date-filed" />
									</el-form-item>
								</div>
							</template>
					...
					<div class="search-filter-form-btns">
						<el-button-group>
							<el-button type="warning" @click="hideSearchFilterBox" icon="Close">取消</el-button>
							<el-button type="success" @click="onSearchFilterSubmit" icon="Search">确定</el-button>
						</el-button-group>
					</div>
				</el-form>
			</div>
					...
					<el-button-group>
						<el-popover placement="bottom" trigger="click">
							...
							<el-scrollbar max-height="400px">
								<el-checkbox v-model="tableColCheckAll" @change="tableColCheckAllChange">全选
								</el-checkbox>
								<template v-for="(item,index) in entityFileds" :key="item.prop">
									<div class="table-col-item" v-if="item.show">
										<el-checkbox :disabled="item.masteShow" :key="item.prop" :label="item.label"
											v-model="item.show_" @change="tableColCheckChange">
											{{item.label }}
										</el-checkbox>
									</div>
								</template>
			...
			<div class="table-content">
				<el-table ref="table" v-loading="tableLoading" :default-sort="sortInfo" :data="tableData" border stripe
					:height="tableHeight" @sort-change="sortChange" style="width: 100%">
						...
						<template v-if="item.masteShow || item.show_">
							<template v-if="item.customer">
								...
								<el-table-column :prop="item.prop" :label="item.label" :width="item.width"
									:fixed="item.fixed" resizable :sortable="item.canSort" :align="item.align">
									<template #default="scope">
										<slot :name="'cell-'+item.prop" :item="item" :index="scope.$index" :row="scope.row"></slot>
									</template>
								</el-table-column>
							</template>
							<template v-else>
								...
								<template v-else-if="item.type == '4' || item.type == '9'">
									<el-table-column :prop="item.prop" :label="item.label" :width="item.width"
										:fixed="item.fixed" resizable :sortable="item.canSort" :align="item.align">
										<template #default="scope">
											{{scope.row[item.prop]?new Date(scope.row[item.prop]).format(item.format?item.format:"YYYY-MM-DD HH:mm:ss"):''}}
										</template>
									</el-table-column>
								</template>
								<template v-else-if="item.type == '5'">
									<template v-if="item.multiple">
										<el-table-column :prop="item.prop" :label="item.label" :width="item.width"
											:fixed="item.fixed" resizable :sortable="item.canSort" :align="item.align">
												<template #default="scope">
													<template v-if="scope.row[item.prop]" v-for="(cellitem,cellindex) in scope.row[item.prop]">
									...
									<template v-else>
										<el-table-column :prop="item.prop" :label="item.label" :width="item.width"
											:fixed="item.fixed" resizable :sortable="item.canSort" :align="item.align">
											<template #default="scope">
												{{scope.row[item.prop]?scope.row[item.prop][item.showLabelKey]:''}}
											</template>
										</el-table-column>
									</template>
								</template>
								<template v-else-if="item.type == '6'">
									<el-table-column :prop="item.prop" :label="item.label" :width="item.width"
										:fixed="item.fixed" resizable :sortable="item.canSort" :align="item.align">
										<template #default="scope">
											<a v-if="scope.row[item.prop]" :href="item.url">{{scope.row[item[prop]]}}</a>
										</template>
									</el-table-column>
								</template>
								...
								<template v-else-if="item.type == '8'">
									<el-table-column :prop="item.prop" :label="item.label" :width="item.width"
										:fixed="item.fixed" resizable :sortable="item.canSort" :align="item.align">
										<template #default="scope">
											<template v-if="item.dicts" v-for="(dictitem,dictindex) in item.dicts"
												:key="dictitem.value">
												<template v-if="dictitem.value == scope.row[item.prop]">
													<el-tag size="small">
														{{dictitem.label}}
													</el-tag>
												</template>
					...
					<el-table-column label="操作" fixed="right" align="center" :width="optionColWidth">
						<template #default="scope">
							<el-button-group>
								<el-button size="small" type="warning" v-if="edit" @click="handleEdit(scope.$index, scope.row)" icon="Edit">
								</el-button>
								<el-button size="small" type="success" @click="handleInfo(scope.$index, scope.row)" icon="View">
								</el-button>
			...
			<div class="table-page">
				<el-pagination v-model:currentPage="pageInfo.pageNo" v-model:page-size="pageInfo.pageSize"
					:page-sizes="[15,30,50,100,200]" background layout="total, sizes, prev, pager, next, jumper"
					:total="pageInfo.totalData" @size-change="pageSizeChange" @current-change="pageNoChange" />
			</div>
		</el-card>
	</div>

	<!-- 编辑表单 -->
	<SmartDialogForm v-if="showForm" :data="curEditData" :url="infourl" :saveurl="saveurl" :entityType="entityType"
		title="编辑数据" @closed="saveEnd">
	</SmartDialogForm>
